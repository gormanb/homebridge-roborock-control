...
import {RoborockAccessory} from './platformAccessory.js';
  ...
  discoverDevices() {
    ...
    for (const device of exampleDevices) {
      ...
      if (existingAccessory) {
        ...
        new RoborockAccessory(this, existingAccessory);

        // it is possible to remove platform accessories at any time using
        // `api.unregisterPlatformAccessories`, e.g.: remove platform
        // accessories when no longer present
        // this.api.unregisterPlatformAccessories(PLUGIN_NAME, PLATFORM_NAME,
        // [existingAccessory]); this.log.info('Removing existing accessory from
        // cache:', existingAccessory.displayName);
      } else {
        ...
        const accessory =
            new this.api.platformAccessory(device.exampleDisplayName, uuid);
        ...
        // this is imported from `platformAccessory.ts`
        new RoborockAccessory(this, accessory);

 ...
 * types.
 */
export class RoborockAccessory {
  ...
  constructor(
      private readonly platform: RoborockControllerPlatform,
      private readonly accessory: PlatformAccessory,
  ) {
    // set accessory information
    this.accessory.getService(this.platform.Service.AccessoryInformation)!
        .setCharacteristic(
            this.platform.Characteristic.Manufacturer, 'Default-Manufacturer')
        .setCharacteristic(this.platform.Characteristic.Model, 'Default-Model')
        .setCharacteristic(
            this.platform.Characteristic.SerialNumber, 'Default-Serial');

    // get the LightBulb service if it exists, otherwise create a new LightBulb
    // service you can create multiple services for each accessory
    ...
    this.service.getCharacteristic(this.platform.Characteristic.On)
        ...
        .onGet(
            this.getOn.bind(this));  // GET - bind to the `getOn` method below

    // register handlers for the Brightness Characteristic
    this.service.getCharacteristic(this.platform.Characteristic.Brightness)
        .onSet(this.setBrightness.bind(
            this));  // SET - bind to the 'setBrightness` method below

    /**
     * Creating multiple services of the same type.
     *
     * To avoid "Cannot add a Service with the same UUID another Service without
     * also defining a unique 'subtype' property." error, when creating multiple
     * services of the same type, you need to use the following syntax to
     * specify a name and subtype id: this.accessory.getService('NAME') ||
     * this.accessory.addService(this.platform.Service.Lightbulb, 'NAME',
     * 'USER_DEFINED_SUBTYPE_ID');
     *
     * The USER_DEFINED_SUBTYPE must be unique to the platform accessory (if you
     * platform exposes multiple accessories, each accessory can use the same
     * subtype id.)
     */
    ...
    const motionSensorOneService =
        ...
        this.accessory.addService(
            this.platform.Service.MotionSensor, 'Motion Sensor One Name',
            'YourUniqueIdentifier-1');

    const motionSensorTwoService =
        this.accessory.getService('Motion Sensor Two Name') ||
        this.accessory.addService(
            this.platform.Service.MotionSensor, 'Motion Sensor Two Name',
     ...
     * asynchronously instead of using the `on('get')` handlers. Here we change
     * update the motion sensor trigger states on and off every 10 seconds the
     * `updateCharacteristic` method.
     *
     */
    ...
    setInterval(() => {
      ...
      motionSensorOneService.updateCharacteristic(
          this.platform.Characteristic.MotionDetected, motionDetected);
      motionSensorTwoService.updateCharacteristic(
          this.platform.Characteristic.MotionDetected, !motionDetected);

      this.platform.log.debug(
          'Triggering motionSensorOneService:', motionDetected);
      this.platform.log.debug(
          'Triggering motionSensorTwoService:', !motionDetected);
    }, 10000);
  }
   ...
   *
   * GET requests should return as fast as possible. A long delay here will
   result in
